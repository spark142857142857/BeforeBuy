...
- **추천 이유를 설명합니다.** 공통 사업 노출과 주요 제품을 근거로 제시합니다.
...
| 영역 | 상태 | 현재 범위 |
| --- | --- | --- |
| 한국 상장 종목 전체 검색 | ✅ 완료 | KOSPI·KOSDAQ·KONEX 2,872개 |
| KRX 업종·주요 제품 연결 | ✅ 완료 | 업종 2,759개, 주요 제품 2,748개 |
| DART 기업 고유번호 매핑 | ✅ 완료 | 상장 종목 2,759개 매핑 |
| 산업 관계 맵 | ✅ 완료 | 7개 섹터의 기업·ETF 관계 탐색 |
| 고급 비교 대시보드 | ✅ 완료 | 국내외 주식·ETF 48개, 비교 관계 19세트 |
| 미국 자산 원화 수익률 | ✅ 화면 검증 | 큐레이션 참고값과 환율 기준일 표시 |
| DART 연간 사업 내용 수집 | ✅ 완료 | 2,739개 사업 프로필 |
| 로컬 유사 종목 계산기 | ✅ 완료 | 2,650개 기업, 상위 후보 26,500개 |
| 전체 종목 국내 peer 연결 | ✅ 완료 | 사업 프로필과 상위 국내 후보 5개 표시 |
| 전체 종목 글로벌 peer 확장 | ✅ 완료 | 7개 테마 규칙으로 한국 종목 500개 연결 |
| ETF 구성 종목 연결 | ✅ 완료 | 국내외 ETF 14개의 주요 편입 종목·기준일·출처 표시 |
| 개별 종목 vs ETF 비교 | ✅ 완료 | 편입 여부·구성 수·집중 위험·환율·성과 지표 비교 |
| 자동 검증·CI | ✅ 완료 | 스냅샷·웹·사용 사례·배치 테스트를 push·PR마다 실행 |
...
- 로딩 중 이전 결과 이동 방지와 방향키·Enter 선택
...
현재 비교 대시보드의 밸류에이션·성과·위험 지표는 데이터 공급자 연결 전 화면
구조를 검증하기 위한 큐레이션 참고값입니다. 실시간 데이터나 자동 갱신값으로
표시하지 않으며, 실제 투자 판단 전 공식 출처에서 최신 값을 확인하도록 안내합니다.
...
- 성공 데이터는 120일 동안 재사용하고 이후 자동 재검사
- 갱신 실패 시 기존 정상 데이터를 유지하고 마지막 시도 상태 기록
...
- 주요 제품·DART 사업 개요에서 다중 사업 노출 태그를 추출하고, KRX 업종은 soft prior로만 반영
- 제품 전용 키워드(`productKeywords`)는 KRX 주요 제품 필드에만 적용하고 DART 본문에는 쓰지 않음
- 2차전지는 약한 공통 `밸류체인` 태그와 셀 제조·소재·부품·장비 역할을 분리해 공급망 단계가 섞이지 않게 함
- 통신 서비스 사업자와 통신장비 제조사를 별도 노출로 구분
- 지주회사는 증권·투자와 분리된 `지주회사` 태그로 설명
- 노출 유사도는 좌·우 포함도와 가중 Jaccard를 함께 사용해 태그 1개 기업 과대평가를 완화
- 제품 유사도는 작은 기업에 과도한 1점이 부여되지 않도록 Dice 점수 사용
- 비교 가능한 peer를 우선하도록 시가총액 비율의 완만한 규모 유사도를 반영하고 화면에서 계산해 표시
- 삼성전자·SK하이닉스·현대차·기아는 DART 사업부문을 근거로 검토된 노출 강도 제공
- 공통 사업 노출과 제품 키워드로 추천 이유 생성
- 공통 사업 노출 없이 낮은 점수로 연결된 장기 꼬리 종목은 `비교 근거 제한적`으로 표시
- 각 종목에 대해 상위 유사 후보와 점수 구성요소 저장
- 삼성전자↔SK하이닉스, 현대차↔기아, NAVER→카카오, LG엔솔→SDI와 통신·금융·조선·화장품 대표 회귀 테스트 적용
...
- 대표 종목의 검수된 ETF 관계와 자동 규칙 ETF를 중복 없이 통합

현재 2,872개 한국 상장 종목 중 근거 기준을 통과한 500개 종목에 글로벌 peer
...
- 미국 ETF의 직접 비교 수익률은 원화 환산값으로 표시
...
웹 화면은 실시간 외부 API를 직접 호출하지 않고, 로컬 배치가 만든 버전 관리 가능한 스냅샷을 읽습니다. 데이터 공급자를 교체할 수 있도록 수집 계층과 화면 계층을 분리했습니다. 현재 토스 인증 정보는 향후 시장 데이터 어댑터를 위한 예약 설정이며 화면 수치 생성에는 사용하지 않습니다.

DART 전체 원문 압축 파일은 로컬 배치 캐시로만 사용하며 GitHub에는 포함하지 않습니다. 웹용 사업 프로필에는 보고서 기간·접수일·원문 URL·신뢰도 같은 메타데이터만 저장하고 본문 미리보기는 포함하지 않습니다. 글로벌 규칙 계산은 로컬 원문의 앞 1,200자를 사용한 뒤 결과만 저장합니다.

이 분리로 `business_profiles.json`을 약 4.25MB에서 0.52MB로, 프로덕션 서버 번들을 약 11.16MB에서 7.59MB로 줄였습니다.
...
| Test & CI | Node test runner, Python unittest, GitHub Actions | 웹 경로·상호작용·데이터 계산·스냅샷 검증 |
...
├── components/
│   ├── SearchExplorer.tsx     # 비동기 검색과 키보드 탐색
│   ├── DomesticPeerSection.tsx
│   ├── GlobalPeerEtfSection.tsx
│   ├── StockEtfComparisonSection.tsx
│   └── IndustryMap.tsx
...
│   ├── prepare_web_profiles.py # 본문을 제외한 웹 메타데이터 생성
...
| `npm run data:dart` | 전체 DART 사업 내용 수집·이어받기·120일 경과 정상본 재검사 |
| `npm run data:profiles` | DART 본문을 제외한 배포용 메타데이터와 우선주 연결 생성 |
| `npm run data:similarity` | 전체 로컬 유사 종목 점수 생성 |
| `npm run data:global` | 글로벌 peer와 관련 ETF 규칙 연결 생성 |
| `npm run data:derive` | 프로필·유사도·글로벌 연결을 순서대로 재생성하고 검증 |
| `npm run data:refresh` | KRX·DART부터 모든 파생 데이터를 안전하게 갱신 |
...
빠르게 회귀 검증합니다. `npm test`는 스냅샷 무결성 검사부터 빌드, 웹 경로,
검색 상태, 사용 사례와 Python 데이터 파이프라인 테스트를 모두 실행합니다.
현재 Node 테스트 22개와 Python 테스트 31개를 통과하며, 같은 검증은 GitHub
Actions에서 `main` push와 pull request마다 실행됩니다.
...
- KOSDAQ GLOBAL 시장 정규화와 대표 종목 검색·상세 흐름
- 전체 유사도 결과의 자기 추천·중복·점수 범위·근거 강도
- 스냅샷 원자적 교체와 실패 시 이전 정상 파일 보존
- 삼성전자·LG에너지솔루션·NAVER 대표 사용 사례
- 우선주의 보통주 사업 프로필 연결과 스팩 제외 사유
- 글로벌·ETF 규칙 미통과 종목의 데이터 공백 표시
- 종목명·종목코드 검색 결과의 상세 URL 일치
- 검색 로딩 중 이전 결과 이동 방지와 키보드 선택 범위
- 비교 후보 최대 4개 제한과 비활성 상태 안내
- 큐레이션·자동 규칙 ETF 통합과 미국 ETF 원화 환산 직접 비교
- 상장 종목 유니버스 변경 시 과거 DART 레코드 격리
